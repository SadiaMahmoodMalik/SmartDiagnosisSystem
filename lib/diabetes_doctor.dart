...
class DiabetesDoctorListPage extends StatefulWidget {
  @override
  _DiabetesDoctorListPageState createState() => _DiabetesDoctorListPageState();
}

class _DiabetesDoctorListPageState extends State<DiabetesDoctorListPage> {
    ...
    _databaseReference.once().then((DatabaseEvent snapshot) {
      if (snapshot.snapshot.value != null) {
        ...
        values.forEach((key, values) {
          ...
          if (doctor.specialization == 'Diabetes Specialist') {
            setState(() {
              doctors.add(doctor);
            });
          }
          ...
          actions: [
            ElevatedButton(
              ...
              onPressed: () {
                ...
                DatabaseReference appointmentsRef = FirebaseDatabase.instance.ref().child('doctors');
                DatabaseReference newAppointmentRef = appointmentsRef.child(doctorEmail).child('appointment-requests');
        newAppointmentRef.child(_currentUser.email!.replaceAll('.', ',')).set("${_currentUser.email} requested an appointment.");
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Diabetes Specialists'),
      ),
